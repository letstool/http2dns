...
[![Docker Pulls](https://img.shields.io/docker/pulls/letstool/http2dns)](https://hub.docker.com/r/letstool/http2dns)

> **DNS-over-HTTP** — Fast & lightweight HTTP gateway that serves DNS queries as a JSON REST API.
...
Provided binaries are fully functional natively on Linux (amd64 or arm64), Windows (amd64), macOS (amd64 or arm64), and also via Docker (amd64 or arm64), with no additional dependencies required. For download and installation, please refer to the [Releases](https://github.com/letstool/http2dns/releases) page.
